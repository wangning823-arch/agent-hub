...
export default function FileViewer({ file, content, onClose, onSave }: FileViewerProps) {
  ...
  const highlightCode = () => {
    if (codeRef.current) {
      codeRef.current.querySelectorAll('pre code').forEach((block: Element) => {
  ...
  const getHljsLanguage = (filename: string): string => {
