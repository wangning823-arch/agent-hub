...
export default function CredentialManager() {
  ...
  const [formData, setFormData] = useState({ host: '', type: 'token', username: '', secret: '', keyData: '' })
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true)
    try {
      const body = { host: formData.host.trim(), type: formData.type }
  ...
  const handleSaveScanned = async (cred) => {
    try {
      const body = { host: cred.host, type: cred.type }
  ...
  const typeLabel = (type) => type === 'ssh' ? 'SSH密钥' : 'Token'
  const typeIcon = (type) => type === 'ssh' ? '🔑' : '🎫'
