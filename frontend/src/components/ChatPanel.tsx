...
interface ChatPanelProps {
  ...
  onOptionsChange?: (options: { mode: string; model: string; effort: string }) => void
  onWorkingChange?: (working: boolean) => void
  onStartingChange?: (starting: boolean) => void
  onSessionLoaded?: () => void
  onSubtaskCountChange?: (info: SubtaskCountInfo) => void
  onSubtaskPanelClose?: () => void
...
export default function ChatPanel({
  ...
  // 引用回复状态
  const [quoteReply, setQuoteReply] = useState<QuoteReply | null>(null)

  // 模型和模式选项
  const [modes, setModes] = useState<OptionItem[]>([])
  ...
  const loadMoreMessages = async () => {
    ...
    try {
      ...
      if (data.messages && data.messages.length > 0) {
        ...
        const filtered = formattedMessages.filter((msg: ChatMessage) => {
    ...
    const loadHistory = async () => {
      ...
      try {
        ...
        if (data.messages && data.messages.length > 0) {
          ...
          // 过滤掉工具调用历史记录和空消息
          const filteredMessages = formattedMessages.filter((msg: ChatMessage) => {
            const content = msg.content
            const contentType = typeof content === 'object' && content !== null ? content.type : null
            if (contentType === 'tool_use' || contentType === 'tool_result') return false
            if (msg.type === 'tool_use' || msg.type === 'tool_result') return false
    ...
    fetch(`${API_BASE}/sessions/${sessionId}/subtasks`)
      ...
      .then(data => {
        if (data.subtasks && data.subtasks.length > 0) {
          ...
          const restored = data.subtasks.map((s: SubtaskData) => ({
            ...s,
         ...
         try {
           ...
           if (msg.subtask_id) {
             if (msg.type === 'text' || msg.type === 'assistant') {
               setSubtasks(prev => prev.map(s => {
                 ...
                 const newMsg: ChatMessage = { type: msg.type, content: msg.content || '', time: Date.now() }
               ...
               setSubtasks(prev => prev.map(s =>
                 s.id === msg.subtask_id
                   ? { ...s, status: (msg.status || s.status) as SubtaskStatus, error: msg.error || s.error }
                   : s
               ))
             }
             return // 不继续处理主消息流
          ...
          if (msg.type === 'tool_use' || msg.type === 'tool_result') {
            ...
            if (msg.type === 'status' && (msg.content === 'task_started' || msg.content === 'task_done')) {
              ...
              const agentNames: Record<string, string> = { 'claude-code': 'Claude', 'opencode': 'OpenCode', 'codex': 'Codex' }
          ...
          if (msg.type === 'context_usage' && msg.content) {
            const parseTokenStr = (s: any): number => {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.success && data.files) {
        const newAttachments: Attachment[] = data.files.map((file: any) => ({
    ...
    if (sendMode === 'split') {
      ...
      try {
        ...
        if (data.shouldSplit && data.subtasks?.length > 0) {
          ...
          setSubtasks(data.subtasks.map((s: SubtaskData) => ({ ...s, messages: s.messages || [] })))
          setShowSubtaskPanel(true)
          sessionStorage.removeItem(`subtaskClosed_${sessionId}`)
          // 将用户消息显示在聊天中
          ...
          <div className="rounded-2xl border overflow-hidden" style={{ background: 'var(--bg-elevated)', borderColor: 'var(--border-primary)' }}>
            ...
            <div className="px-3 pt-2 relative">
              ...
              <textarea
                ...
                style={{ color: 'var(--text-primary)', minHeight: 40, maxHeight: 120, opacity: (isWorking || isStarting || isRestoringMemory || splitAnalyzing) ? 0.5 : 1, touchAction: 'manipulation' } as React.CSSProperties}
                rows={1}
              />
            </div>
            {/* 底部工具栏 */}
