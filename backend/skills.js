 ...
 * 获取翻译后的描述
 */
function getTranslatedDescription(skill) {
 ...
 * 扫描目录获取 Skills 列表
 */
function scanSkillsDir(dirPath, basePath = '') {
  const skills = [];
 ...
 */
function getSkillsForAgent(agentType) {
  let skills;
  switch (agentType) {
    case 'claude-code':
    case 'claude-api':
 ...
 */
async function installClaudeSkill(source, options = {}) {
  const { scope = 'user' } = options;
  
  // 支持的源格式:
  // 1. GitHub: owner/repo 或 owner/repo#branch
  // 2. Git URL: https://... 或 git://...
  // 3. 远程 marketplace.json URL
 ...
 * 安装 OpenCode Plugin
 */
async function installOpenCodePlugin(pluginName) {
  return new Promise((resolve, reject) => {
    try {
 ...
 */
async function uninstallClaudePlugin(pluginName) {
  return new Promise((resolve, reject) => {
    try {
